#!/usr/bin/env bash
# Bootstrap script for one-command ROADFX deployment (China Network Optimized)
#
...
#
# Note: After deployment, use './roadfx.sh install --cn' to pull Docker images
...
REPO="${REPO:-https://gitee.com/ivansslo/roadfx.git}"
DIR="${DIR:-roadfx}"
REF="${REF:-}"
...
main() {
  ...
  if [ -f "./roadfx.sh" ] && [ -f "./docker-compose.yml" ]; then
    echo "[INFO] Detected existing roadfx-deploy checkout in $(pwd). Running ./roadfx.sh install --cn..."
  ...
  if [ -f "$DIR/roadfx.sh" ]; then
    echo "[RUN] (cd $DIR && ./roadfx.sh install --cn)"
    (cd "$DIR" && ./roadfx.sh install --cn)
  else
