# ROADFX Dev Environment Redesign
...
ROADFXs current local development flow is split across multiple modes:
...
This gives ROADFX one explicit development path while preserving clean configuration boundaries:
...
- core backend services: `roadfx-api`, `roadfx-ai`, `roadfx-rag`, `roadfx-platform`, `roadfx-workflow`, `roadfx-plugin-runtime`, `roadfx-device-control`
- core frontend services: `roadfx-web`, `roadfx-widget-js`
...
- `http://roadfx-api:8000`
- `http://roadfx-ai:8081`
- `http://roadfx-rag:8082`
...
4. `roadfx-platform`
5. `roadfx-workflow`
6. `roadfx-plugin-runtime`
7. `roadfx-device-control`
...
- `roadfx-web` is reachable on `http://localhost:5173`
- `roadfx-widget-js` is reachable on `http://localhost:5174`
- `roadfx-api` and `roadfx-ai` health endpoints respond on their published localhost ports
...
- `make logs SERVICE=roadfx-api` and `make restart SERVICE=roadfx-ai` work as the main operational escape hatches
