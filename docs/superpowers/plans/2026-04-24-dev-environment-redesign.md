# ROADFX Dev Environment Redesign Implementation Plan
...
docker compose --env-file .env.dev -f docker-compose.yml -f docker-compose.source.yml config > /tmp/roadfx-dev.before.yml
```

Expected: command succeeds and `/tmp/roadfx-dev.before.yml` contains `build:` sections for the source-mode services.
...
- `roadfx-ai`
- `roadfx-api`
- `roadfx-plugin-runtime`
- `roadfx-device-control`
- `roadfx-platform`
- `roadfx-workflow`
- `roadfx-workflow-worker`
...
volumes:
  ...
  roadfx-ai-venv:
  roadfx-rag-venv:
  roadfx-platform-venv:
  roadfx-workflow-venv:
  roadfx-plugin-runtime-venv:
  roadfx-device-control-venv:
  roadfx-web-node-modules:
  roadfx-widget-node-modules:
```
...
```bash
docker compose --env-file .env.dev -f docker-compose.yml config > /tmp/roadfx-base.after.yml
```
...
REDIS_HOST=redis
REDIS_PORT=6379
API_SERVICE_URL=http://roadfx-api:8000
AI_SERVICE_URL=http://roadfx-ai:8081
RAG_SERVICE_URL=http://roadfx-rag:8082
PLATFORM_SERVICE_URL=http://roadfx-platform:8003
WORKFLOW_SERVICE_URL=http://roadfx-workflow:8000
PLUGIN_RUNTIME_URL=http://roadfx-plugin-runtime:8090
DEVICE_CONTROL_SERVICE_URL=http://roadfx-device-control:8085
```
...
ROADFXAI_PORT=8081
ROADFXRAG_PORT=8082
ROADFXPLATFORM_PORT=8003
ROADFXWORKFLOW_PORT=8004
ROADFXPLUGIN_RUNTIME_PORT=8090
ROADFXDEVICE_CONTROL_PORT=8085
ROADFXWEB_PORT=5173
ROADFXWIDGET_PORT=5174
```
...
roadfx-api:
  env_file:
    - .env.dev
  volumes:
    - ./repos/roadfx-api:/app
    - roadfx-api-venv:/app/.venv
    - ./data/roadfx-api/uploads:/app/uploads
  ports:
    - "${ROADFXAPI_PORT:-8000}:8000"
  command:
...
```bash
docker compose --env-file .env.dev -f docker-compose.yml -f docker-compose.dev.yml config > /tmp/roadfx-backend-dev.yml
./scripts/dev/init.sh --dry-run
```
...
- `/tmp/roadfx-backend-dev.yml` includes `migrate-api` through `migrate-device`
...
- [ ] **Step 2: Override `roadfx-web` and `roadfx-widget-js` to run Vite inside dev containers**
...
roadfx-web:
  ...
  env_file:
    - .env.dev
  volumes:
    - ./repos/roadfx-web:/app
    - roadfx-web-node-modules:/app/node_modules
  ports:
    - "${ROADFXWEB_PORT:-5173}:5173"
  command:
    - sh
    - -lc
    - |
...
Mirror that pattern for `roadfx-widget-js` on port `5174`.
...
- keep `roadfx-device-control-agentos` behind `agentos`
...
```bash
docker compose --env-file .env.dev -f docker-compose.yml -f docker-compose.dev.yml config > /tmp/roadfx-frontend-dev.yml
rg -n "node:20-alpine|roadfx-web-node-modules|roadfx-widget-node-modules|monitoring|agentos" /tmp/roadfx-frontend-dev.yml
```
...
```make
DEV_SERVICES := postgres redis wukongim roadfx-rag roadfx-rag-worker roadfx-rag-beat roadfx-ai roadfx-plugin-runtime roadfx-device-control roadfx-platform roadfx-workflow roadfx-workflow-worker roadfx-api roadfx-web roadfx-widget-js
...
```make
dev: check-env
	@$(COMPOSE) build roadfx-rag roadfx-ai roadfx-api roadfx-plugin-runtime roadfx-device-control roadfx-platform roadfx-workflow
	@$(COMPOSE) up -d $(INFRA_SERVICES)
	@ENV_FILE=$(ENV_FILE) ./scripts/dev/init.sh
	@$(COMPOSE) up -d $(RUN_SERVICES)
	@echo "ROADFX dev environment is ready."
```
...
```bash
timeout 5 make logs SERVICE=roadfx-api || true
make restart SERVICE=roadfx-ai
./scripts/test-dev-environment.sh
```
...
- restart recreates only `roadfx-ai`
...
```bash
touch repos/roadfx-api/app/main.py
docker compose --env-file .env.dev -f docker-compose.yml -f docker-compose.dev.yml logs --since=20s roadfx-api
```
...
```bash
echo "// dev-hmr-check $(date +%s)" >> repos/roadfx-web/src/main.tsx
docker compose --env-file .env.dev -f docker-compose.yml -f docker-compose.dev.yml logs --since=20s roadfx-web
python3 - <<'PY'
from pathlib import Path
path = Path("repos/roadfx-web/src/main.tsx")
