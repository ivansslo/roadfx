#!/usr/bin/env bash
# Bootstrap script for one-command ROADFX deployment
...
REPO="${REPO:-https://github.com/ivansslo/roadfx.git}"
DIR="${DIR:-roadfx}"
REF="${REF:-}"
...
install_docker() {
  ...
  if [ "$OS_TYPE" = "macos" ]; then
    ...
    if command -v getent >/dev/null 2>&1 && getent group docker >/dev/null 2>&1; then
      ...
      # remember this so we can avoid running roadfx.sh in the same session,
...
update_image_tags() {
  ...
  echo "[INFO] Setting Docker image tags to '$image_tag' for ROADFX application services..."

  if [ -f "$compose_main" ]; then
    echo "[INFO] Updating $compose_main ..."
    sed -i.bak -E "s#(ghcr.io/ivansslo/roadfx/roadfx-[^:]+:)[^[:space:]]+#\1${image_tag}#g" "$compose_main" || {
  ...
  if [ -f "$compose_cn" ]; then
    ...
    sed -i.bak -E "s#(registry.cn-shanghai.aliyuncs.com/ivansslo/roadfx-[^:]+:)[^[:space:]]+#\1${image_tag}#g" "$compose_cn" || {
...
main() {
  ...
  # continuing into roadfx.sh install, which would hit permission errors.
  detect_os
  if [ "$OS_TYPE" = "linux" ] && [ "${DOCKER_GROUP_JUST_ADDED:-0}" -eq 1 ]; then
    ...
    echo "  1) Log out and log back in, then run (from your roadfx-deploy repo directory):" >&2
    echo "       ./roadfx.sh install" >&2
    echo "     OR" >&2
  ...
  # If we're already inside a roadfx-deploy working dir, run roadfx.sh install
  if [ -f "./roadfx.sh" ] && [ -f "./docker-compose.yml" ]; then
    echo "[INFO] Detected existing roadfx-deploy checkout in $(pwd). Running ./roadfx.sh install..."
    ./roadfx.sh install
    return
  fi


  # Otherwise, clone the repo to DIR and run roadfx.sh install
  if [ -d "$DIR/.git" ]; then
  ...
  if [ -f "$DIR/roadfx.sh" ]; then
    echo "[RUN] (cd $DIR && ./roadfx.sh install)"
    (cd "$DIR" && ./roadfx.sh install)
  else
    echo "[FATAL] Neither roadfx.sh nor deploy.sh found in $DIR" >&2
    exit 1
  fi
