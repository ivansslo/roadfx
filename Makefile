...
BUILD_SERVICES := roadfx-rag roadfx-ai roadfx-api roadfx-plugin-runtime roadfx-device-control roadfx-platform roadfx-workflow
CORE_APP_SERVICES := roadfx-rag roadfx-rag-worker roadfx-rag-beat roadfx-ai roadfx-plugin-runtime roadfx-device-control roadfx-platform roadfx-workflow roadfx-workflow-worker roadfx-api roadfx-web roadfx-widget-js
MONITORING_SERVICES := roadfx-celery-flower adminer
AGENTOS_SERVICES := roadfx-device-control-agentos
...
help:
	@echo ""
	@echo "$(CYAN)ROADFX Dev Commands$(RESET)"
	@echo "================="
	@echo ""
	@echo "  make dev                       Build backend images, run init, and launch the dev stack"
	@echo "  make down                      Stop the dev stack"
	@echo "  make logs [SERVICE=roadfx-api]    Follow logs for all services or one service"
	@echo "  make ps                        Show current compose service status"
	@echo "  make restart SERVICE=roadfx-ai    Restart one service"
	...
	@echo "  make dev DISABLE=roadfx-rag-beat,roadfx-workflow-worker"
	@echo ""
...
restart: check-env
	@if [ -z "$(SERVICE)" ]; then \
		echo "$(RED)Usage: make restart SERVICE=roadfx-ai$(RESET)"; \
		exit 1; \
	fi
