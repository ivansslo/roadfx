...
ENV_FILE=".env"
CONFIG_FILE="./data/.roadfx-install-mode"
...
run_all_migrations() {
  ...
  echo "[INFO] Running Alembic migrations for roadfx-rag..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-rag alembic upgrade head
  
  echo "[INFO] Running Alembic migrations for roadfx-ai..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-ai alembic upgrade head
  
  echo "[INFO] Running Alembic migrations for roadfx-api..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-api alembic upgrade head
  
  echo "[INFO] Running Alembic migrations for roadfx-platform..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T -e PYTHONPATH=. roadfx-platform alembic upgrade head

  echo "[INFO] Running Alembic migrations for roadfx-workflow..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-workflow alembic upgrade head

  echo "[INFO] Running Alembic migrations for roadfx-plugin-runtime..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-plugin-runtime alembic upgrade head

  echo "[INFO] Running Alembic migrations for roadfx-device-control..."
  docker compose --env-file "$ENV_FILE" $compose_file_args run --rm -T roadfx-device-control alembic upgrade head
}
...
# Check if a port is used by the ROADFX nginx container
# Returns 0 if used by roadfx-nginx, 1 otherwise
is_port_used_by_roadfx_nginx() {
  local port="$1"

  # Check if roadfx-nginx container is running
  if ! docker ps --filter "name=roadfx-nginx" --filter "status=running" --format "{{.Names}}" 2>/dev/null | grep -q "roadfx-nginx"; then
    return 1  # Container not running
  fi

  # Check if roadfx-nginx is using this port
  # Method 1: Check docker port mappings
  if docker port roadfx-nginx 2>/dev/null | grep -E "^(80|443)/tcp -> .*:${port}$" >/dev/null; then
    return 0  # roadfx-nginx is using this port
  fi

  # Method 2: Check container's published ports in docker inspect
  if docker inspect roadfx-nginx --format='{{range $p, $conf := .NetworkSettings.Ports}}{{range $conf}}{{.HostPort}}{{"\n"}}{{end}}{{end}}' 2>/dev/null | grep -q "^${port}$"; then
    return 0  # roadfx-nginx is using this port
  fi

  return 1  # Port not used by roadfx-nginx
}
...
check_and_configure_ports() {
  ...
  if is_port_in_use "$http_port"; then
    if is_port_used_by_roadfx_nginx "$http_port"; then
      echo "  ✓ Port $http_port is used by ROADFX Nginx (OK for re-install)"
    else
  ...
  if is_port_in_use "$https_port"; then
    if is_port_used_by_roadfx_nginx "$https_port"; then
      echo "  ✓ Port $https_port is used by ROADFX Nginx (OK for re-install)"
    else
  ...
  if [ "$http_port_changed" = true ] || [ "$https_port_changed" = true ]; then
    ...
    echo "ROADFX has been configured to use the following ports:"
    echo ""
...
usage() {
  cat <<'EOF'
Usage: ./roadfx.sh <command> [options]
...
Notes:
  ...
  - Options can be combined: ./roadfx.sh install --source --cn
...
Domain Configuration Examples:
  ...
  ./roadfx.sh config ssl_mode auto
  ./roadfx.sh config ssl_email admin@example.com
  ./roadfx.sh config setup_letsencrypt
  ./roadfx.sh config show

Upgrade Command:
  - The upgrade command remembers the mode used during install (saved in ./data/.roadfx-install-mode)
  - If you provide --source or --cn flags, they will override the saved configuration
  - Examples:
    • ./roadfx.sh install --cn          → ./roadfx.sh upgrade (auto-uses --cn)
    • ./roadfx.sh install --source      → ./roadfx.sh upgrade (auto-uses --source)
    • ./roadfx.sh upgrade --cn          → Override to use --cn for this upgrade
EOF
}
...
save_install_mode() {
  ...
  # Write configuration
  cat > "$CONFIG_FILE" << EOF
# ROADFX Install Mode Configuration
# Auto-generated by ./roadfx.sh install
# This file is used by ./roadfx.sh upgrade to remember your deployment preferences
...
ensure_env_files() {
  if [ ! -f "$ENV_FILE" ]; then
    if [ -f ".env.example" ]; then
      ...
      cat > "$ENV_FILE" << 'ENVEOF'
# ROADFX Environment Configuration
# Auto-generated by ./roadfx.sh install
...
POSTGRES_PORT=5432
POSTGRES_DB=roadfx
POSTGRES_USER=roadfx
POSTGRES_PASSWORD=roadfx
...
has_domain_config() {
  local domain_config_file="./data/.roadfx-domain-config"
...
configure_server_host() {
  ...
  # Check if user has already configured domains via ./roadfx.sh config
...
update_frontend_urls() {
  ...
  local widget_script_base="${protocol}://${host}${port_suffix}/widget/roadfx-widget-sdk.js"
...
configure_core_settings() {
  ...
  echo ""
  echo "  PostgreSQL: ${POSTGRES_USER:-roadfx}@${POSTGRES_HOST:-postgres}:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-roadfx}"
...
show_install_complete_message() {
  ...
  echo "========================================="
  echo ""
  echo "ROADFX has been successfully installed!"
  echo ""
  echo "Access URLs:"
  ...
  echo "  • Stop services:  ./roadfx.sh down"
  echo "  • Restart:        ./roadfx.sh up"
  echo "  • Upgrade:        ./roadfx.sh upgrade"
  echo ""
  echo "Documentation: https://roadfx.ai"
  echo ""
...
wait_for_postgres() {
  ...
  local retries=30
  local user="${POSTGRES_USER:-roadfx}"
  local db="${POSTGRES_DB:-roadfx}"
  local attempt=0
...
cmd_install() {
  ...
  DATA_DIRS=(
    ...
    "./data/redis"
    "./data/wukongim"
    "./data/roadfx-rag/uploads"
    "./data/roadfx-api/uploads"
    "./data/roadfx-workflow/uploads"
    "./data/roadfx-device-control/screenshots"
    "./data/nginx"
    "./data/skills"
...
cmd_upgrade() {
  ...
  if [ -n "$md5_before" ] && [ "$md5_before" != "$md5_after" ]; then
    echo "[INFO] roadfx.sh has been updated. Re-executing with the new version..."
...
cmd_build() {
  ...
  case "$target" in
    api) services=(roadfx-api) ;;
    rag) services=(roadfx-rag roadfx-rag-worker roadfx-rag-beat) ;;
    workflow) services=(roadfx-workflow roadfx-workflow-worker) ;;
    ai) services=(roadfx-ai) ;;
    plugin-runtime) services=(roadfx-plugin-runtime) ;;
    device-control) services=(roadfx-device-control) ;;
    platform) services=(roadfx-platform) ;;
    celery) services=(roadfx-celery-flower) ;;
    web) services=(roadfx-web) ;;
    widget) services=(roadfx-widget-js) ;;
    all) services=() ;;
    *)
...
update_domain_env_vars() {
  ...
  if [ -n "$widget_domain" ] && [ "$widget_domain" != "localhost" ]; then
    ...
    update_env_var "VITE_WIDGET_SCRIPT_BASE" "${protocol}://${widget_domain}/roadfx-widget-sdk.js"
    ...
    echo "[INFO] Updated VITE_WIDGET_SCRIPT_BASE=${protocol}://${widget_domain}/roadfx-widget-sdk.js"
...
cmd_config() {
  ...
  case "$subcommand" in
    ...
    ssl_manual)
      if [ $# -lt 2 ]; then
        echo "[ERROR] Usage: ./roadfx.sh config ssl_manual <cert_file> <key_file> [domain]"
        exit 1
      fi
      local cert_file="$1"
      local key_file="$2"
    ...
    setup_letsencrypt)
      ...
      if [ -z "$WEB_DOMAIN" ] || [ -z "$WIDGET_DOMAIN" ] || [ -z "$API_DOMAIN" ]; then
        ...
        echo "[INFO] Run: ./roadfx.sh config web_domain <domain>"
        exit 1
      fi
    ...
    apply)
      ...
      if docker compose --env-file "$ENV_FILE" $compose_file_args ps --quiet 2>/dev/null | grep -q .; then
        ...
        echo "[INFO] Run './roadfx.sh up' to start services with new configuration."
      fi
      ;;
    show)
      if [ ! -f "$domain_config_file" ]; then
        echo "[INFO] No domain configuration found"
        echo "[INFO] Run: ./roadfx.sh config web_domain <domain> to get started"
        exit 0
      fi
    ...
    *)
      ...
      echo "Usage: ./roadfx.sh config <subcommand> [args]"
      echo ""
      echo "Subcommands:"
...
# ROADFX Domain Configuration
# Auto-generated by ./roadfx.sh config
...
cmd_doctor() {
  ...
  local services=(
    "roadfx-postgres:PostgreSQL"
    "roadfx-redis:Redis"
    "roadfx-wukongim:WuKongIM"
    "roadfx-rag:RAG Service"
    "roadfx-rag-worker:RAG Worker"
    "roadfx-rag-beat:RAG Beat"
    "roadfx-celery-flower:Celery Flower"
    "roadfx-ai:AI Service"
    "roadfx-plugin-runtime:Plugin Runtime"
    "roadfx-device-control:Device Control"
    "roadfx-workflow:Workflow Service"
    "roadfx-workflow-worker:Workflow Worker"
    "roadfx-api:API Service"
    "roadfx-platform:Platform Service"
    "roadfx-web:Web Frontend"
    "roadfx-widget-js:Widget App"
    "roadfx-nginx:Nginx"
  )
  ...
  if [ "$all_healthy" = true ]; then
    ...
    echo "  • Restart all:    ./roadfx.sh down && ./roadfx.sh up"
    echo "  • Check status:   docker compose ps"
    echo ""
    return 1
  fi
