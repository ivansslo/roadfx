...
for PAIR in "8000:roadfx-api" "8081:roadfx-ai"; do
  PORT="${PAIR%%:*}"
  NAME="${PAIR#*:}"
  if is_port_open "$PORT"; then
    printf "  ✓ %-20s :%s\n" "$NAME" "$PORT"
...
for PAIR in "18082:roadfx-rag" "8003:roadfx-platform" "8004:roadfx-workflow" "8090:roadfx-plugin-runtime" "8085:roadfx-device-control" "5173:roadfx-web" "5174:roadfx-widget-js"; do
  PORT="${PAIR%%:*}"
  NAME="${PAIR#*:}"
  if is_port_open "$PORT"; then
    printf "  ✓ %-20s :%s\n" "$NAME" "$PORT"
