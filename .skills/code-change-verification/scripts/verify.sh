...
for SERVICE in $SERVICES; do
  ...
  case "$SERVICE" in
    ...
    roadfx-ai|roadfx-api|roadfx-rag|roadfx-platform|roadfx-workflow|roadfx-plugin-runtime|roadfx-device-control)
      cd "$REPO_ROOT/$SERVICE_DIR"
      if [ -f "pyproject.toml" ]; then
        ...
        if poetry run flake8 "$SRC_DIR" 2>/dev/null; then
          ...
          # Try ruff as fallback (roadfx-platform uses ruff)
