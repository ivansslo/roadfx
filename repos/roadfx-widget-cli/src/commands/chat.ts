...
export async function chatSend(params: {
  ...
  if (!server) throw new Error('No server configured. Run: roadfx-widget init');
  if (!apiKey) throw new Error('No API key configured. Run: roadfx-widget init');
  if (!config.platform_open_id) throw new Error('No platform_open_id. Run: roadfx-widget init');
...
export async function chatSendWs(params: {
  ...
  const config = loadConfig();
  if (!config.channel_id) throw new Error('No channel_id. Run: roadfx-widget init');
  if (config.channel_type == null) throw new Error('No channel_type. Run: roadfx-widget init');
