...
export function registerInitCommand(parent: Command): void {
  parent
    ...
    .action(async (opts, cmd) => {
      ...
      if (!apiKey) {
        printError(new Error('API key required. Use: roadfx-widget -k <key> init --server <url>'), format);
        return;
      }
      try {
...
export async function initVisitor(params: {
  ...
  if (!server) throw new Error('No server specified. Use --server <url> or set ROADFXWIDGET_SERVER');
  ...
  body.system_info = {
    ...
    source_detail: `roadfx-widget-cli/0.1.0 on ${hostname()}`,
  };
  body.source = 'roadfx-widget-cli';
