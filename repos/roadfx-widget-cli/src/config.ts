...
const CONFIG_DIR = join(homedir(), '.roadfx-widget');
...
export function resolveServer(flag?: string): string | undefined {
  return flag || process.env.ROADFXWIDGET_SERVER || loadConfig().server;
}

export function resolveApiKey(flag?: string): string | undefined {
  return flag || process.env.ROADFXWIDGET_API_KEY || loadConfig().api_key;
}

export function resolveOutput(flag?: string): 'json' | 'table' | 'compact' {
  const v = flag || process.env.ROADFXWIDGET_OUTPUT || loadConfig().output;
