...
被控端 Agent — 运行在受管设备上，通过 TCP 长连接接入 `roadfx-device-control` 服务，向 AI Agent 提供文件读写、编辑和 Shell 执行能力。

## 功能特性

- **TCP JSON-RPC 2.0** 长连接，与 `roadfx-device-control` 实时通信
...
```bash
./bin/roadfx-device-agent --config configs/config.json
```
...
```
roadfx-ai (Agent) ──MCP──> roadfx-device-control ──TCP──> roadfx-device-agent
    │                         │                          │
