...
class ROADFXAPIException(Exception):
    """Base exception for ROADFX API."""
...
class ValidationError(ROADFXAPIException):
...
class NotFoundError(ROADFXAPIException):
...
class ConflictError(ROADFXAPIException):
...
class AuthenticationError(ROADFXAPIException):
...
class AuthorizationError(ROADFXAPIException):
...
class RateLimitError(ROADFXAPIException):
...
class ExternalServiceError(ROADFXAPIException):
...
async def roadfx_api_exception_handler(request: Request, exc: ROADFXAPIException) -> JSONResponse:
    """Handle ROADFX API exceptions."""
    request_id = str(uuid4())
    
    logger.error(
        f"ROADFX API Exception: {exc.code} - {exc.message}",
        extra={
