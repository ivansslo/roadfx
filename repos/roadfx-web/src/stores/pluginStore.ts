...
export const usePluginStore = create<PluginState>((set, get) => ({
  ...
  handlePluginAction: (pluginId, action, context) => {
    ...
    switch (type) {
      ...
      case 'insert_text':
        if (data.text) {
          window.dispatchEvent(new CustomEvent('roadfx:insert_text', { detail: { text: data.text, replace: data.replace } }));
        }
        break;

      case 'send_message':
        if (data.content) {
          window.dispatchEvent(new CustomEvent('roadfx:send_message', { detail: { content: data.content, content_type: data.content_type } }));
        }
        break;

      case 'show_toast':
        if (data.message) {
          window.dispatchEvent(new CustomEvent('roadfx:show_toast', { detail: { message: data.message, type: data.type || 'success' } }));
        }
        break;
