...
const storeAuthFetch = async <T>(
  ...
  // 添加 ROADFX API 的 token
  const roadfxToken = localStorage.getItem('roadfx-auth-token');
  if (roadfxToken) {
    (headers as Record<string, string>)['Authorization'] = `Bearer ${roadfxToken}`;
  }
...
export const storeApi = {
  ...
  installModel: async (id: string) => {
    // 调用 ROADFX API 同步模型到本地项目 (ROADFX API 内部会处理商店状态)
  ...
  getInstalledModels: async (): Promise<string[]> => {
    // 从本地 ROADFX API 获取已安装模型列表 (model_id 列表)
