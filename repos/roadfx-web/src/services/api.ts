/**
 * API Service Layer for ROADFX Web
...
class APIClient {
  ...
  constructor(baseURL: string) {
    ...
    this.token = localStorage.getItem('roadfx-auth-token');
  }
  ...
  setToken(token: string | null) {
    this.token = token;
    if (token) {
      localStorage.setItem('roadfx-auth-token', token);
    } else {
      localStorage.removeItem('roadfx-auth-token');
    }
  }
