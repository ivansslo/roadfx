...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({ variant = 'icon', placement = 'bottom', usePortal = true }) => {
  ...
  const [pref, setPref] = useState<string>(() => {
    try { return localStorage.getItem('roadfx-language') || 'system'; } catch { return 'system'; }
  });
  ...
  const handleLanguageChange = (languageCode: string) => {
    if (languageCode === 'system' || languageCode === 'auto') {
      ...
      try { localStorage.setItem('roadfx-language', 'system'); } catch {}
      try { document.documentElement.setAttribute('lang', resolved); } catch {}
      setPref('system');
    } else {
      i18n.changeLanguage(languageCode);
      try { localStorage.setItem('roadfx-language', languageCode); } catch {}
