"""Seed script: Register roadfx-device-control as an MCP tool in the ai_tools table.

Usage:
    python -m scripts.seed_device_control_tool --project-id <UUID>

This creates a LOCAL MCP tool record pointing to the roadfx-device-control
...
DEVICE_CONTROL_ENDPOINT = "http://roadfx-device-control:8085/mcp/{device_id}"
    ...
    if not db_url:
        try:
            ...
            print(
                ...
                "Pass --database-url or run from the roadfx-ai directory.",
                file=sys.stderr,
            )
            sys.exit(1)
