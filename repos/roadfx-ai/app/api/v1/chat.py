    ...
    200: {
        ...
        "content": {
            ...
            "text/event-stream": {
                "schema": {
                    "type": "string",
                    "description": "Server-Sent Events (SSE) stream in OpenAI format with ROADFXAI tool extensions.",
                },
                "examples": {
                    "streaming_response": {
...
### 2. ROADFXAI 扩展参数 (ROADFXAI Extensions)
...
#### 流式模式 (stream=true)
返回 `text/event-stream`。除了标准的 OpenAI chunk 外，还会包含 ROADFXAI 自定义的工具执行事件：

| 事件类型 (SSE Data) | 描述 |
| :--- | :--- |
| `{"id":"...","object":"chat.completion.chunk",...}` | 标准的文本增量或模型原始输出。 |
| `{"type": "tool_call", "tool_call": {...}}` | **ROADFXAI 自定义事件**：通知前端后端正在自动执行某个工具调用。 |
| `{"type": "tool_result", "tool_call_id": "...", "result": "..."}` | **ROADFXAI 自定义事件**：工具执行完成，返回执行结果。 |
| `data: [DONE]` | 流结束。 |
