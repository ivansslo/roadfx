# roadfx-widget-js — AGENTS.md

> Stack: React 18 + TypeScript 5.6 + Vite 5 + Emotion + Zustand 5 · Port: 5174
> Sister project: `roadfx-web` — json-render components must stay in sync
...
- json-render changes must check if `roadfx-web` needs sync, and vice versa
- Styles use Emotion + inline CSSProperties — no Tailwind (that's roadfx-web)
...
- `postMessage` event types and payload structure must stay compatible (`roadfx:visibility`, `ROADFXWIDGET_UNREAD`, `ROADFXSHOW_TOAST`, `ROADFXWIDGET_CONFIG`)
...
## json-render sync with roadfx-web
...
# Functional — widget-app calls roadfx-api visitor endpoints; use CLI to verify
ROADFX_WIDGET_CLI="node ../roadfx-widget-cli/dist/index.js"
$ROADFX_WIDGET_CLI platform info               # platform config API
$ROADFX_WIDGET_CLI channel info                # channel API
$ROADFX_WIDGET_CLI chat history --limit 3      # message history API
$ROADFX_WIDGET_CLI chat send --message "say ok" --no-stream  # chat e2e
```
