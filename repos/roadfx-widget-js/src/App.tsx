...
export default function App(){
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      const d = e.data
      if (d && d.type === 'roadfx:visibility' && typeof d.open === 'boolean') {
        setVisible(d.open)
        if (d.open) {
          clearUnreadCount()
        }
  ...
  useEffect(() => {
    console.log('[App] Sending ROADFXWIDGET_UNREAD to parent, count:', unreadCount)
    try {
      window.parent?.postMessage({ type: 'ROADFXWIDGET_UNREAD', payload: { count: unreadCount } }, '*')
    } catch (e) {
      console.error('[App] Failed to send ROADFXWIDGET_UNREAD:', e)
    }
  }, [unreadCount])
  ...
  useEffect(()=>{
    ...
    try { window.parent?.postMessage?.({ type: 'ROADFXWIDGET_CONFIG', payload: { theme_color: theme } }, '*') } catch {}
  }, [theme])

  // Notify host page to position the UI iframe
  useEffect(()=>{
    try { window.parent?.postMessage?.({ type: 'ROADFXWIDGET_CONFIG', payload: { position: pos } }, '*') } catch {}
  }, [pos])

  // Notify host page about expanded state
  useEffect(()=>{
    try { window.parent?.postMessage?.({ type: 'ROADFXWIDGET_CONFIG', payload: { expanded: !!isExpanded } }, '*') } catch {}
  }, [isExpanded])
  ...
  useEffect(()=>{
    ...
    const SS_STARTED_AT = 'roadfx_session_started_at'
    const SS_PAGES = 'roadfx_session_pages'
    try {
    ...
    const onMsg = (e: MessageEvent) => {
      ...
      if (d?.type === 'ROADFXHOST_PAGE_INFO' && d.payload?.page_url) {
      ...
      } else if (d?.type === 'ROADFXHOST_PAGE_EXIT') {
        flush('exit')
      ...
      } else if (d?.type === 'ROADFXWIDGET_SHOWN') {
        try { useChatStore.getState().clearUnreadCount() } catch {}
      } else if (d?.type === 'ROADFXTRACK_EVENT' && d.payload) {
        const uid = useChatStore.getState().myUid || myUid
        const apiKey = resolveApiKey()
        if (!cfg.apiBase || !uid || !apiKey) return
        const p = d.payload || {}
    ...
    if (useChatStore.getState().myUid || myUid) {
      try { window.parent?.postMessage?.({ type: 'ROADFXREQUEST_PAGE_INFO' }, '*') } catch {}
    }

    // We rely on host SDK to send ROADFXHOST_PAGE_EXIT; avoid double triggering here.
  ...
  const requestClose = () => {
    try { (window.parent as any)?.ROADFXWidget?.hide?.(); } catch {}
  };
