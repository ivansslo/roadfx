...
function getTargetDocument(): Document {
  try {
    const isController = window.name === 'roadfx-controller-frame'
    if (isController && window.parent && 'frames' in window.parent) {
      const uiFrame = (window.parent as any).frames['roadfx-ui-frame'] as Window | undefined
...
const container = targetDoc.getElementById('roadfx-root') || targetDoc.getElementById('root')
...
const emotionCache = createCache({ key: 'roadfx', container: targetDoc.head })

ReactDOM.createRoot((targetDoc.getElementById('roadfx-root') || targetDoc.getElementById('root')) as HTMLElement).render(
  <React.StrictMode>
    <CacheProvider value={emotionCache}>
      <Global styles={css`
        :root { --primary:#2f80ed; --bg:#ffffff; --text:#1f2937; --muted:#6b7280; }
        html, body, #roadfx-root, #root { height: 100%; overscroll-behavior: contain; }
