...
class TelegramChannelListener:
    ...
    def __init__(
        ...
        roadfx_api_client: TgoApiClient,
        sse_manager: SSEManager,
    ) -> None:
        self._session_factory = session_factory
        self._normalizer = normalizer
        self._roadfx_api_client = roadfx_api_client
    ...
    async def _get_or_register_visitor(
        ...
        # Register or get visitor via roadfx-api
        if platform.api_key:
            try:
    ...
    async def _process_update(
        ...
        async with self._session_factory() as db:
            try:
                # 1) Get/register visitor FIRST to get the official UUID from roadfx-api
                visitor, display_name, avatar_url = await self._get_or_register_visitor(platform, msg_data)
                
                # 2) If it's an image, download from Telegram and upload to roadfx-api
                    ...
                    try:
                        ...
                        if file_info.get("ok"):
                            ...
                            if file_path:
                                ...
                                # c) Upload to roadfx-api using the real visitor.channel_id
                                ...
                                async with httpx.AsyncClient(timeout=60) as client:
                                    ...
                                    if resp.status_code == 200:
                                        ...
                                        print(f"[TELEGRAM] Failed to upload image to roadfx-api (status {resp.status_code}): {resp.text}")
                    except Exception as e:
                ...
                reply_text = await process_message(
                    msg=msg,
                    db=db,
                    roadfx_api_client=self._roadfx_api_client,
