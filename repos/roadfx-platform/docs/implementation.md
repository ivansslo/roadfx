## roadfx-platform Implementation Guide (Python/FastAPI)
...
选型理由
- 全异步链路：渠道监听 → roadfx-api（SSE）→ 平台适配，需 async/await 优先（FastAPI、httpx、SQLAlchemy asyncio）。
...
app/
  ...
  domain/
    ...
    services/
      __init__.py
      dispatcher.py          # 调用 roadfx-api 客户端
...
职责：调用 roadfx-api /v1/wukongim/messages/send-stream 建立 SSE，产出事件流。
```python
...
职责：
- 解析 roadfx-api 的 SSE：connected|event|error|disconnected
...
class Settings(BaseSettings):
    ...
    api_base_url: str  # roadfx-api base url
...
async def orchestrate(msg: NormalizedMessage, db: AsyncSession) -> None:
    """端到端编排：查库取平台配置（含 api_key）→ 调用 roadfx-api（SSE）→ 平台适配输出。"""
    for attempt in range(3):  # 简单重试
        try:
            ...
            frames = roadfx_api_client.send_stream(req)
...
async def debug_pipe():
    async def event_gen():
        async for ev in sse_manager.stream_events(roadfx_api_client.send_stream(...)):
...
.env.example
```
API_BASE_URL=https://roadfx-api.example.com

DATABASE_URL=postgresql+asyncpg://user:pass@127.0.0.1:5432/roadfx
...
- 可靠性：
  - roadfx-api 超时使用 timeout_seconds，与平台适配回退策略对齐
...
async def process_message(msg: NormalizedMessage, db: AsyncSession) -> None:
    for attempt in range(3):  # 简单重试（指数退避）
        try:
            ...
            frames = roadfx_api_client.send_stream(req_body)
            events = sse_manager.stream_events(frames)
