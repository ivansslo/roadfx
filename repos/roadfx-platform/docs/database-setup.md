...
This guide covers database initialization, migrations, and common operations for the roadfx-platform project.
...
```
postgresql+asyncpg://roadfx_user:roadfx_pass@127.0.0.1:5442/roadfx
```

### From Docker Network (for containerized apps)
```
postgresql+asyncpg://roadfx_user:roadfx_pass@postgres:5432/roadfx
```

**Important**: The roadfx-platform app runs on the host machine, so it uses `127.0.0.1:5442`.
...
### "role roadfx_user does not exist"
...
**Solution**: Update `.env`:
```bash
PG_DSN=postgresql+asyncpg://roadfx_user:roadfx_pass@127.0.0.1:5442/roadfx
```
