"""Plugin Tool Sync Service - Sync plugin tools to roadfx-ai service."""
...
class PluginToolSyncService:
    ...
    async def sync_plugin_tools(self, plugin: PluginConnection):
        """
        Sync plugin tools to roadfx-ai when a plugin connects.
        
        This creates/updates tool records in roadfx-ai for each MCP tool the plugin provides.
        """
    ...
    async def remove_plugin_tools(self, plugin_id: str):
        """
        Remove plugin tools from roadfx-ai when a plugin disconnects.
        """
    ...
    async def _register_tool(self, tool_data: Dict[str, Any]):
        """Register a tool in roadfx-ai (create or update across all projects)."""
