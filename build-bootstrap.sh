...
sed -e 's|^# Bootstrap script for one-command ROADFX deployment$|# Bootstrap script for one-command ROADFX deployment (China Network Optimized)\n#\n# This is the China-optimized version of bootstrap.sh that uses Gitee mirrors\n# for faster Git repository cloning in mainland China.\n#\n# Differences from bootstrap.sh:\n# - Uses Gitee (https://gitee.com) instead of GitHub for repository cloning\n# - All other functionality remains identical\n#\n# Note: After deployment, use '\''./roadfx.sh install --cn'\'' to pull Docker images\n# from Alibaba Cloud Container Registry for faster downloads in China.|' \
    ...
    -e 's|^REPO="${REPO:-https://github.com/ivansslo/roadfx.git}"$|REPO="${REPO:-https://gitee.com/ivansslo/roadfx.git}"|' \
    -e 's|\./roadfx\.sh install\.\.\.|\./roadfx.sh install --cn...|g' \
    -e 's|\./roadfx\.sh install"|\./roadfx.sh install --cn"|g' \
    -e 's|\./roadfx\.sh install)|\./roadfx.sh install --cn)|g' \
    -e 's|\./roadfx\.sh install$|\./roadfx.sh install --cn|g' \
...
echo "  ✓ Install command: ./roadfx.sh install → ./roadfx.sh install --cn"
