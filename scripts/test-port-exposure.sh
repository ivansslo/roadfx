...
if ! grep -A 5 "roadfx-api:" docker-compose.yml | grep -q "ports:"; then
  echo "  ✅ roadfx-api 没有暴露端口"
else
  echo "  ❌ roadfx-api 仍然暴露端口"
  exit 1
fi
echo ""

# Test 2: Check roadfx-web ports
echo "✓ 测试 2: 检查 roadfx-web 是否暴露端口"
if ! grep -A 10 "roadfx-web:" docker-compose.yml | grep -q "ports:"; then
  echo "  ✅ roadfx-web 没有暴露端口"
else
  echo "  ❌ roadfx-web 仍然暴露端口"
  exit 1
fi
echo ""

# Test 3: Check roadfx-widget-js ports
echo "✓ 测试 3: 检查 roadfx-widget-js 是否暴露端口"
if ! grep -A 10 "roadfx-widget-js:" docker-compose.yml | grep -q "ports:"; then
  echo "  ✅ roadfx-widget-js 没有暴露端口"
else
  echo "  ❌ roadfx-widget-js 仍然暴露端口"
  exit 1
fi
echo ""
...
if grep -q "API_BASE_URL=http://roadfx-api:8000" .env.example; then
...
if grep -q "VITE_WIDGET_PREVIEW_URL=http://roadfx-widget-js:80" .env.example; then
...
echo "✓ 测试 8: 检查反向代理配置"
if grep -q "proxy_pass http://roadfx-api:8000" scripts/generate-nginx-config.sh; then
  echo "  ✅ 反向代理配置正确"
else
  echo "  ❌ 反向代理配置不正确"
  exit 1
...
echo ""
echo "端口暴露策略验证完成："
echo "  • roadfx-api 不暴露端口"
echo "  • roadfx-web 不暴露端口"
echo "  • roadfx-widget-js 不暴露端口"
