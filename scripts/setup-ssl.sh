...
if ! docker ps --format '{{.Names}}' | grep -q "^roadfx-nginx$"; then
    echo "[ERROR] roadfx-nginx container is not running"
    echo "[INFO] Please start ROADFX services first: ./roadfx.sh up"
    exit 1
fi
...
if [ $SUCCESS_COUNT -gt 0 ]; then
    ...
    docker restart roadfx-nginx >/dev/null 2>&1 && echo "[INFO] Nginx restarted successfully" || echo "[WARN] Could not restart nginx, please run: docker restart roadfx-nginx"
fi
