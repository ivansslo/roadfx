...
CONFIG_FILE="$PROJECT_ROOT/data/.roadfx-domain-config"
...
# -----------------------------------------------------------------------------
# Upload / Proxy settings (for /api -> roadfx-api)
...
map $http_referer $assets_upstream {
    ~*/widget(/|$)  roadfx-widget-js:80;
    default          roadfx-web:80;
}
...
NGINX_CONFIG
    ...
    location ~ ^/api(/|$) {
        rewrite ^/api(/.*)$ $1 break;
        proxy_pass http://roadfx-api:8000;
    ...
    location ~ ^/widget(/.*)?$ {
        ...
        rewrite ^$ / break;
        proxy_pass http://roadfx-widget-js:80;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    # Decide upstream (roadfx-web vs roadfx-widget-js) based on Referer
    location /assets/ {
    ...
    # Choose upstream (roadfx-web or roadfx-widget-js) based on Referer
    location / {
        proxy_pass http://$assets_upstream;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
...
server {
    ...
    # In localhost unified mode, /v1 would otherwise go to roadfx-web, causing Swagger to load HTML instead of OpenAPI JSON.
    location ~ ^/v1(/|$) {
        proxy_pass http://roadfx-api:8000;
        # Upload settings (configured via .env)
        client_max_body_size CLIENT_MAX_BODY_SIZE;
