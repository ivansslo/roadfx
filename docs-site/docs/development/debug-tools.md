...
ROADFX 内置了一些调试工具，帮助你排查问题和监控系统状态。
...
| 系统 | PostgreSQL |
| 服务器 | `postgres` |
| 用户名 | `roadfx`（或查看 `.env` 中的 `POSTGRES_USER`） |
| 密码 | `roadfx`（或查看 `.env` 中的 `POSTGRES_PASSWORD`） |
| 数据库 | `roadfx`（或查看 `.env` 中的 `POSTGRES_DB`） |
...
2. 检查 roadfx-api 日志是否有错误
3. 确认 Kafka 服务正常运行

### API 请求失败

1. 查看 roadfx-api 日志
2. 使用 `./roadfx.sh doctor` 检查服务状态
3. 检查 Nginx 配置和日志
...
1. 不要将调试工具端口暴露到公网
2. 使用完毕后及时关闭：`./roadfx.sh tools stop`
3. 修改默认的数据库密码
:::
