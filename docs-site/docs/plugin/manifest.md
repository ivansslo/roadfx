...
`plugin.yml` 是 ROADFX 插件的“身份卡”，它定义了插件的基本信息、源码位置以及 ROADFX 如何构建和运行该插件。它是实现插件自动化部署、升级的核心文件。
...
author: ROADFX Team                 # 作者

# 源码配置 (支持 GitHub/Gitee)
source:
  github:
    repo: ivansslo/my-plugin-repo   # 仓库路径
    path: ./                     # 插件代码在仓库中的子路径
...
### runtime (运行时)
- **command**: ROADFX 默认会根据语言自动生成启动指令（如 `go run` 或 `python main.py`）。如果需要特殊参数，可以在此自定义。
...
## 如何验证
在安装插件时，只需提供 `plugin.yml` 的 URL，ROADFX 会实时解析并显示该文件的内容。如果解析失败，请检查 YAML 语法是否正确。
