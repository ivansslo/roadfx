...
ROADFX 插件系统允许开发者通过标准化的接口扩展客服系统的功能。插件可以为访客面板添加自定义信息展示、扩展聊天工具栏、集成第三方应用等。

## 什么是 ROADFX 插件

ROADFX 插件是一个独立运行的进程，通过 **Unix Socket** 与 ROADFX 宿主程序进行通讯。
...
5. **[模版规范](/plugin/templates)**：了解 ROADFX 支持的所有 UI 组件模版。
6. **[完整示例](/plugin/examples)**：通过真实的业务示例学习如何构建复杂的插件。

## 核心概念

ROADFX 插件系统的交互基于以下核心概念：
...
- **plugin.yml (配置文件)**：插件的身份卡。定义了插件的 ID、版本、源码地址、构建方式及运行参数。它是 ROADFX 自动化管理插件的核心。
