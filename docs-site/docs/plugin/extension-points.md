...
ROADFX 提供多个扩展点，允许插件在不同位置扩展客服系统功能。
...
func (p *MyPlugin) OnVisitorPanelRender(ctx *roadfx.RenderContext) roadfx.Template {
	return roadfx.NewGroup().
		Add(roadfx.NewKeyValue("客户信息").
			Add("级别", "VIP", roadfx.KeyValueIcon("crown"), roadfx.KeyValueColor("#FFD700")).
			Add("最近消费", ctx.Visitor.Metadata["total_spent"])).
		Add(roadfx.NewButton("查看 CRM", "view_crm").SetSize("sm"))
}

func (p *MyPlugin) OnVisitorPanelEvent(ctx *roadfx.EventContext) *roadfx.Action {
	if ctx.ActionID == "view_crm" {
		return roadfx.OpenURL("https://crm.example.com/" + ctx.VisitorID, "_blank")
	}
	return roadfx.Noop()
}
```
